...
if realm list | grep -q .; then
    echo "✅ Domain Membership: OK"
    ((SUCCESS_COUNT++))
else
    echo "❌ Domain Membership: FAILED"
    ((ERROR_COUNT++))
fi
...
if getent passwd | grep -q "@"; then
    echo "✅ Domain Users: ACCESSIBLE"
    ((SUCCESS_COUNT++))
else
    echo "⚠️  Domain Users: NOT FOUND (may be normal)"
    ((WARNING_COUNT++))
fi
...
if [[ -f /var/log/sssd/sssd.log ]] && [[ $(tail -50 /var/log/sssd/sssd.log | grep -i error | wc -l) -eq 0 ]]; then
    echo "✅ SSSD Logs: CLEAN"
    ((SUCCESS_COUNT++))
else
    echo "⚠️  SSSD Logs: ERRORS DETECTED"
    ((WARNING_COUNT++))
fi
